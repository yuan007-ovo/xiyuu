...
window.addEventListener('ChatDBReady', () => {
    ...
    if (savedMiniMax.key) document.getElementById('minimaxApiKey').value = savedMiniMax.key;
    if (savedMiniMax.groupId) document.getElementById('minimaxGroupId').value = savedMiniMax.groupId;

    // 恢复 MCP 配置
    const mcpAddress = localStorage.getItem('mcpAddress') || '';
    const mcpRoleId = localStorage.getItem('mcpRoleId') || '';
    const addrInput = document.getElementById('mcpAddressInput');
    const roleInput = document.getElementById('mcpRoleIdInput');
    if (addrInput) addrInput.value = mcpAddress;
    if (roleInput) roleInput.value = mcpRoleId;
});
...
function saveAndCloseApiPanel() {
    ...
    const minimaxKey = document.getElementById('minimaxApiKey').value.trim();
    ...
    ChatDB.setItem('minimax_api_config', JSON.stringify({ key: minimaxKey, groupId: minimaxGroupId }));
...
function captureFullState() {
    ...
    const appIds = ['app1', 'app2', 'app3', 'app4', 'app-dock1', 'app-dock-music', 'app-mall', 'app-dock-call', 'app-dock-sms', 'app-weibo', 'app-alipay', 'app-day', 'app-sys-browser'];
