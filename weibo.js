...
window.NPC_AVATARS = [
    ...
    "https://i.postimg.cc/FKpmyh4H/1000000837.jpg",
    "https://i.postimg.cc/85nSYwsV/1000000862.jpg",
    "https://i.postimg.cc/LsDRtmp6/1000018766.jpg",
    "https://i.postimg.cc/Bv44PXd4/1000019708.jpg",
    "https://i.postimg.cc/nLpp9C6F/1000021547.jpg",
    "https://i.postimg.cc/bvppSs43/1000021920.png",
    "https://i.postimg.cc/25rrLVgT/1000021929.jpg",
    "https://i.postimg.cc/pdRR5p3C/1000021930.jpg",
    "https://i.postimg.cc/VkjzNjZq/1000022056.jpg",
    "https://i.postimg.cc/4xzX3z2y/1000022115.jpg",
    "https://i.postimg.cc/Sx0yqZdX/1000022262.jpg",
    "https://i.postimg.cc/13x9PWBw/1000022437.jpg",
    "https://i.postimg.cc/7ZvxDB9b/1000022665.jpg",
    "https://i.postimg.cc/QNy8YNW4/1000024374.jpg",
    "https://i.postimg.cc/sgFVynmR/1000024375.jpg",
    "https://i.postimg.cc/c4tspqQT/1000024552.jpg",
    "https://i.postimg.cc/qqSpNQTf/1000024817.jpg",
    "https://i.postimg.cc/yxt7Dnzr/1000024823.jpg",
    "https://i.postimg.cc/x8h9Xg2D/1000024837.jpg",
    "https://i.postimg.cc/66mWb6G6/1000024838.jpg",
    "https://i.postimg.cc/7YJxFvgg/1000024839.jpg",
    "https://i.postimg.cc/3rc8frDL/1000024841.jpg",
    "https://i.postimg.cc/90rWXKsZ/1000029505.jpg",
    "https://i.postimg.cc/6qgB8zNv/1000029513.webp",
    "https://i.postimg.cc/J0G17vSB/1000031949.jpg",
    "https://i.postimg.cc/ZnCTY2XK/1000031950.jpg",
    "https://i.postimg.cc/Kz9G2xq0/1000032879.jpg",
    "https://i.postimg.cc/LXy9p2Cy/1000032880.jpg",
    "https://i.postimg.cc/NMNscBJz/1000032881.jpg",
    "https://i.postimg.cc/tTrqjpSc/1000032882.jpg",
    "https://i.postimg.cc/FRn46G1r/1000032911.jpg",
    "https://i.postimg.cc/B6ZSkXgm/1000036384.jpg",
    "https://i.postimg.cc/j5qxmLXZ/1000036726.jpg",
    "https://i.postimg.cc/fLWzpJK9/1000037035.jpg",
    "https://i.postimg.cc/MTZ6NvD7/1000037036.jpg",
...
function submitWeiboPost() {
    ...
    topics.forEach(topic => {
        let topicPosts = JSON.parse(ChatDB.getItem(`weibo_super_topic_posts_${weiboAccountId}_${topic}`) || '[]');
        topicPosts.unshift(newPost);
        ChatDB.setItem(`weibo_super_topic_posts_${weiboAccountId}_${topic}`, JSON.stringify(topicPosts));
    });
...
必须返回严格的 JSON 格式，不要有任何 markdown 标记，格式如下：
{
    ...
    try {
        ...
        let weiboNpcs = JSON.parse(ChatDB.getItem(`weibo_npcs_${currentLoginId}`) || '[]');
        ...
        ChatDB.setItem(`weibo_npcs_${currentLoginId}`, JSON.stringify(weiboNpcs));
...
function deleteWeiboPost(postId, event) {
    ...
    if (document.getElementById('weiboSuperTopicPanel').style.display === 'flex' && superTitleEl) {
        ...
        let topicPosts = JSON.parse(ChatDB.getItem(`weibo_super_topic_posts_${currentLoginId}_${topic}`) || '[]');
        if (!postToDelete) postToDelete = topicPosts.find(p => p.id === postId);
        topicPosts = topicPosts.filter(p => p.id !== postId);
        ChatDB.setItem(`weibo_super_topic_posts_${currentLoginId}_${topic}`, JSON.stringify(topicPosts));
    ...
    posts.forEach((post, index) => {
        ...
        itemEl.innerHTML = `
            ...
            <div class="wb-post-actions">
                <div class="wb-action-item" onclick="openWeiboShareChatModal('${post.id}', event)"><svg viewBox="0 0 24 24"><path d="M17 3H7c-1.1 0-2 .9-2 2v16l7-3 7 3V5c0-1.1-.9-2-2-2z"/></svg> ${post.forwards || '转发'}</div>
...
function renderWeiboHotBoard() {
    ...
    container.innerHTML = '';
    let hotTopics = JSON.parse(ChatDB.getItem(`weibo_hot_topics_${currentLoginId}`) || '[]');
...
必须返回严格的 JSON 格式，不要有任何 markdown 标记，格式如下：
{
    ...
    try {
        ...
        ChatDB.setItem(`weibo_videos_${currentLoginId}`, JSON.stringify(data.videos));
        ChatDB.setItem(`weibo_hot_topics_${currentLoginId}`, JSON.stringify(data.hotTopics));
...
必须返回严格的 JSON 格式，不要有任何 markdown 标记，格式如下：
{
    ...
    try {
        ...
        data.topics.forEach(t => {
            t.img = typeof window.getRandomNpcAvatar === 'function' ? window.getRandomNpcAvatar() : `https://source.unsplash.com/random/200x200/?${t.imgKeyword || 'scenery'}`;
        });

        let existingTopics = JSON.parse(ChatDB.getItem(`weibo_super_topics_${currentLoginId}`) || '[]');
        existingTopics = [...data.topics, ...existingTopics];
        ChatDB.setItem(`weibo_super_topics_${currentLoginId}`, JSON.stringify(existingTopics));
...
async function generateWeiboSuperTopicPostsAPI() {
    ...
    const currentLoginId = ChatDB.getItem('weibo_current_account');
    let topics = JSON.parse(ChatDB.getItem(`weibo_super_topics_${currentLoginId}`) || '[]');
...
必须返回严格的 JSON 格式，不要有任何 markdown 标记，格式如下：
{
    ...
    try {
        ...
        let existingPosts = JSON.parse(ChatDB.getItem(`weibo_super_topic_posts_${currentLoginId}_${topicName}`) || '[]');
        existingPosts = [...data.posts, ...existingPosts];
        ChatDB.setItem(`weibo_super_topic_posts_${currentLoginId}_${topicName}`, JSON.stringify(existingPosts));
...
function confirmCreateWeiboSuperTopic() {
    ...
    if (currentEditingSuperTopic) {
        ...
        ChatDB.setItem(`weibo_super_topics_${currentLoginId}`, JSON.stringify(topics));
        
        // 如果修改了名称，需要迁移旧帖子数据
        if (name !== currentEditingSuperTopic) {
            const oldPosts = ChatDB.getItem(`weibo_super_topic_posts_${currentLoginId}_${currentEditingSuperTopic}`);
            if (oldPosts) {
                ChatDB.setItem(`weibo_super_topic_posts_${currentLoginId}_${name}`, oldPosts);
                ChatDB.removeItem(`weibo_super_topic_posts_${currentLoginId}_${currentEditingSuperTopic}`);
            }
        }
...
function openWeiboHotDetail(topicTitle) {
    ...
    const randomCover = typeof window.getRandomNpcAvatar === 'function' ? window.getRandomNpcAvatar() : 'https://images.unsplash.com/photo-1494438639946-1ebd1d20bf85?q=80&w=600&auto=format&fit=crop';
...
function renderWeiboMockFeed(containerId, topic) {
    ...
    const currentLoginId = ChatDB.getItem('weibo_current_account');
    let posts = JSON.parse(ChatDB.getItem(`weibo_super_topic_posts_${currentLoginId}_${topic}`) || '[]');
...
let isWeiboSuperTopicEditing = false;

function toggleWeiboSuperTopicEditMode() {
    isWeiboSuperTopicEditing = !isWeiboSuperTopicEditing;
    const btn = document.getElementById('wbSuperTopicEditBtn');
    if (isWeiboSuperTopicEditing) {
        btn.innerText = '完成';
...
function deleteWeiboSuperTopicFromDiscover(topicTitle, event) {
    event.stopPropagation();
    if (confirm(`确定要删除超话 [${topicTitle}] 吗？相关的帖子也会被删除。`)) {
        const currentLoginId = ChatDB.getItem('weibo_current_account');
        let topics = JSON.parse(ChatDB.getItem(`weibo_super_topics_${currentLoginId}`) || '[]');
        topics = topics.filter(t => t.title !== topicTitle);
        ChatDB.setItem(`weibo_super_topics_${currentLoginId}`, JSON.stringify(topics));
        
        ChatDB.removeItem(`weibo_super_topic_posts_${currentLoginId}_${topicTitle}`);
...
function initWeiboDiscoverSuperTopics() {
    ...
    topics.forEach((topic, index) => {
        ...
        myItem.className = `wb-super-topic-item ${isWeiboSuperTopicEditing ? 'is-editing' : ''}`;
        myItem.style.position = 'relative';
        
        if (isWeiboSuperTopicEditing) {
            myItem.innerHTML = `
                <div class="wb-super-topic-delete-btn" onclick="deleteWeiboSuperTopicFromDiscover('${topic.title}', event)">×</div>
...
confirmCreateWeiboSuperTopic = function() {
    ...
    topics = JSON.parse(ChatDB.getItem(`weibo_super_topics_${currentLoginId}`) || '[]');
...
let currentShareWeiboPostId = null;

function openWeiboShareChatModal(postId, event) {
    // 阻止事件冒泡，防止触发进入详情页
    if (event) event.stopPropagation();
    
    currentShareWeiboPostId = postId;
    const currentLoginId = ChatDB.getItem('weibo_current_account');
    if (!currentLoginId) return alert('请先登录微博！');
    ...
    const listEl = document.getElementById('weiboShareContactList');
    listEl.innerHTML = '';
    ...
    if (friends.length === 0) {
        ...
        friends.forEach(f => {
            ...
            `;
            item.onclick = () => confirmWeiboShareToChat(f.id);
            listEl.appendChild(item);
        });
    }
    
    document.getElementById('weiboShareChatModalOverlay').classList.add('show');
}

function closeWeiboShareChatModal() {
    document.getElementById('weiboShareChatModalOverlay').classList.remove('show');
    currentShareWeiboPostId = null;
}

function confirmWeiboShareToChat(targetCharId) {
    const currentLoginId = ChatDB.getItem('weibo_current_account');
    let posts = JSON.parse(ChatDB.getItem(`weibo_posts_${currentLoginId}`) || '[]');
    
    // 如果在主页没找到，去超话里找
    let post = posts.find(p => p.id === currentShareWeiboPostId);
    if (!post) {
        let topics = JSON.parse(ChatDB.getItem(`weibo_super_topics_${currentLoginId}`) || '[]');
        for (let t of topics) {
            let topicPosts = JSON.parse(ChatDB.getItem(`weibo_super_topic_posts_${currentLoginId}_${t.title}`) || '[]');
            post = topicPosts.find(p => p.id === currentShareWeiboPostId);
            if (post) break;
        }
    }
    
    if (!post) return alert('微博不存在！');

    let contentText = post.content || '';
    if (contentText.length > 50) contentText = contentText.substring(0, 50) + '...';
    
    // 构造分享卡片 HTML
    const encodedPost = encodeURIComponent(JSON.stringify(post));
    const shareHtml = `
        <div class="weibo-share-card" onclick="event.stopPropagation(); showWeiboShareDetail('${encodedPost}')" style="width: 220px; background: #fff; border-radius: 12px; border: 1px solid #eee; box-shadow: 0 4px 15px rgba(0,0,0,0.05); overflow: hidden; cursor: pointer;">
            <div style="padding: 12px; display: flex; align-items: center; gap: 8px; border-bottom: 1px solid #f5f5f5;">
                <div style="width: 24px; height: 24px; border-radius: 50%; background-image: url('${post.authorAvatar}'); background-size: cover; background-position: center; background-color: #eee;"></div>
                <div style="font-size: 13px; font-weight: bold; color: #333;">${post.authorName}</div>
            </div>
            <div style="padding: 12px; font-size: 13px; color: #555; line-height: 1.5;">
                ${contentText}
            </div>
            <div style="padding: 8px 12px; background: #fafafa; font-size: 11px; color: #888; border-top: 1px solid #f5f5f5; display: flex; align-items: center; gap: 4px;">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="#ff8200" stroke-width="2" fill="none"><path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"></path></svg>
                微博
            </div>
        </div>
    `;
    ...
    let newMsg = { 
        ...
        weiboData: {
            ...
            images: post.images && post.images.length > 0 ? post.images.map(img => img.type === 'desc' ? img.text : '真实图片').join(', ') : '无',
            comments: post.commentsList && post.commentsList.length > 0 ? post.commentsList.map(c => `${c.authorName}: ${c.content}`).join(' | ') : '无'
        },
        timestamp: Date.now() 
    };
    ...
    alert('分享成功！');
    closeWeiboShareChatModal();
}
function showWeiboShareDetail(encodedPost) {
    try {
        const post = JSON.parse(decodeURIComponent(encodedPost));
        
        document.getElementById('wsdAvatar').style.backgroundImage = `url('${post.authorAvatar}')`;
        document.getElementById('wsdName').innerText = post.authorName;
        document.getElementById('wsdContent').innerHTML = (post.content || '').replace(/\n/g, '<br>');
        
        const imagesContainer = document.getElementById('wsdImages');
        imagesContainer.innerHTML = '';
        if (post.images && post.images.length > 0) {
            post.images.forEach(img => {
                if (img.type === 'local') {
                    imagesContainer.innerHTML += `<div style="aspect-ratio: 1; background-image: url('${img.url}'); background-size: cover; background-position: center; border-radius: 4px; border: 1px solid #eee;"></div>`;
                } else {
                    imagesContainer.innerHTML += `<div style="aspect-ratio: 1; display: flex; justify-content: center; align-items: center; padding: 4px; background: #f0f0f0; border-radius: 4px; border: 1px solid #eee;"><span style="font-size: 10px; color: #666; text-align: center; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden;">${img.text}</span></div>`;
                }
            });
        }
        
        document.getElementById('wsdForwards').innerText = post.forwards || '转发';
        document.getElementById('wsdComments').innerText = post.comments || '评论';
        document.getElementById('wsdLikes').innerText = post.likes || '赞';
        
        // 渲染评论
        const commentsList = document.getElementById('wsdCommentsList');
            ...
            post.commentsList.forEach(c => {
                ...
                commentsList.innerHTML += `
                    <div style="display: flex; gap: 10px;">
                        <div style="width: 28px; height: 28px; border-radius: 50%; background-image: url('${c.authorAvatar}'); background-size: cover; background-position: center; flex-shrink: 0; background-color: #eee;"></div>
                        <div style="flex: 1; display: flex; flex-direction: column; gap: 2px;">
                            <div style="font-size: 12px; color: #576b95; font-weight: bold;">${c.authorName}</div>
                            <div style="font-size: 13px; color: #333; line-height: 1.4;">${replyHtml}${c.content}</div>
                        </div>
                    </div>
                `;
            });
        } else {
            commentsList.innerHTML = '<div style="text-align: center; color: #888; font-size: 12px; padding: 10px 0;">暂无评论</div>';
        }
        
        document.getElementById('weiboShareDetailModalOverlay').classList.add('show');
    } catch (e) {
        console.error("解析微博分享数据失败", e);
    }
}

function closeWeiboShareDetail() {
    document.getElementById('weiboShareDetailModalOverlay').classList.remove('show');
}
